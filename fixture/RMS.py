...
class rms:
    ...
    def computation_rms(self):
        ...
        h2 = screenshot_outOfRangeMaxConnectionsLimitTariff.histogram()
        h3 = screenshot_progress_bar.histogram()
        h4 = screenshot_deviceNotConnectedToServer.histogram()
        h5 = screenshot_outOfRangeMaxConnectionsLimitTariff_and_focus.histogram()
        ...
        self.rms4 = math.sqrt(reduce(operator.add, map(lambda a, b: (a - b) ** 2, h1, h5)) / len(h1))  # определяем разницу между скриншотом плеера и скриншотом серверной ошибки Превышено максимальное подключение с наведенным курсором
