...
class Messages:
    ...
    def login_blocked(self):
        print('Проверка того, что произошла блокировка входа в личный канет. Проверка прошла успешно. Вход в личный кабинет заблокирован')
        with open('authorization report.txt', 'a', encoding='utf-8') as f:
            f.write(
                '"' + self.app.DateDetermination.strg_today + '" INFO: Проверка, что после ввода пяти раз не корректного пароля происходит блокировка входа в личный кабинет прошла успешно. Отобразилась надпись "Вход в личный кабинет заблокирован.".\n')
            f.close()

    def no_lock_occurred(self):
        print('Проверка того, что произошла блокировка входа в личный канет. Проверка провалилась. Блокировка не произошла')
        with open('authorization error report.txt', 'a', encoding='utf-8') as f:
            f.write(
