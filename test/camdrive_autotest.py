...
def test_log_out(app):
    app.Authorization.login_with_incorrect_data() #Проверка того, что указан не вернный пароль
    app.Authorization.password_visibility_check() #Проверка, что при нажатии на "глаз" видно введенный пароль
    app.Authorization.login_with_valid_data() #Проверка того, что указан вернный логин и пароль
